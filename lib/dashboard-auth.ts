...
const AUTH_TIMEOUT_MS = 20000 // 20s – voorkom oneindig laden bij cold start / trage Supabase
 ...
 * On timeout (e.g. Railway cold start) returns session: null so user gets redirect naar login.
 */
export async function getDashboardAuth(): Promise<DashboardAuthResult> {
  const timeoutPromise = new Promise<DashboardAuthResult>((resolve) =>
    setTimeout(
      () =>
        resolve({
          session: null,
  ...
  const work = async (): Promise<DashboardAuthResult> => {
